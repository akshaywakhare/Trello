        ...
        ref.get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                ...
                for(String comp:dashboard.keySet())
                {
                    ...
                    {
                        myLists.add(new MyList(R.mipmap.groceries_foreground,"Groceries"));
                    }
                    else if(comp.equalsIgnoreCase("To-do List"))
                    {
                        myLists.add(new MyList(R.mipmap.todo_foreground,"To-do List"));
                    }
                    else if(comp.equalsIgnoreCase("Meetings"))
                    {
                        myLists.add(new MyList(R.mipmap.call_foreground,"Meetings"));
                    }
                    else if(comp.equalsIgnoreCase("Furniture"))
                        myLists.add(new MyList(R.mipmap.furniture_foreground,"Furniture"));
                    else if(comp.equalsIgnoreCase("Kitchen Appliances"))
                        myLists.add(new MyList(R.mipmap.kitchen_foreground,"Kitchen Appliances"));
                    else
        ...
        listener=new MyAdapter.RecyclerViewClickListener() {
            ...
            public void OnClick(View v, int position) {
                if(myLists.get(position).getDesc().equalsIgnoreCase("Groceries"))
                {
                    ...
                    intent.putExtra("des","Groceries");
                    intent.putExtra("email",email);
                    startActivity(intent);
                }
                if(myLists.get(position).getDesc().equalsIgnoreCase("To-do List"))
                {
                    Intent intent=new Intent(getApplicationContext(),Todo.class);
                    intent.putExtra("email",email);
                    startActivity(intent);
                }
                if(myLists.get(position).getDesc().equalsIgnoreCase("Meetings"))
                {
